...
%     Assume That the camera movement is uniform in the camera. Orientation is fixed. height is fixed. 
    ...
    ODO_OpticalFlow2Dpos = [-mean(double(OpticalFlow_u(i,1:matched_feat(i))))*pixelsize*X(3)/f  mean(double(OpticalFlow_v(i,1:matched_feat(i))))*pixelsize*X(3)/f];
    
%     tests movements in 2D
    figure(1)
    title('Optical Flow Motion in 3D');
...
% test remove outliers 
    ...
    figure(4)
    title('Optical Flow Motion in 2D')
