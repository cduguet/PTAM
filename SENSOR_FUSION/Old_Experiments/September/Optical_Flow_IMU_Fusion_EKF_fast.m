...
sigma_nba = [0.01 0.01 0.01];        var_nba = sigma_nba.^2;
% sigma_nba = [0 0 0];
...
%scale noise
var_nL = 0.05^2; % relative to unit

% Covariance Matrix of the Optical Flow measurements
R = diag([40 40 90].^2); %mm2
...
%State vector itself
X = repmat(X0,length(IMUTime),1);
X_exp = repmat(X0,length(IMUTime),1);

X_err0 = zeros(1,22);
X_err = repmat(X_err0,length(IMUTime),1);
...
firstOFmeasurement = TRUE;

for  i = 2:IMUTime_instants
    delta_t_IMU = diff(IMUTime(i-1:i));
    %% PROPAGATION
    ...
    [X_exp(i,:),omega_m(i,:),a_m(i,:)] = IMU_Strapdown(X(i-1,:),IMUrotvel(i-1:i,:),IMUacc(i-1:i,:),IMUTime(i-1:i),ViconPose(i+2,:));
    
    % --------------------- Expected State Values -----------------------------
    p_w_i_hat       = X_exp(i-1,1:3);
    v_w_i_hat       = X_exp(i-1,4:6);
    q_w_i_hat       = X_exp(i-1,7:10);  % Expectation fof the quaternion from world to IMU
    % q_w_i_hat = quatmult(ODO_IMUrot,X(7:10));
    % q_w_i_hat = quat_normalize(q_w_i_hat);
    b_omega_hat     = X_exp(i-1,11:13); % Expectation of the bias of the IMU gyro measurements
    b_a_hat         = X_exp(i-1,14:16); % Expectation of the bias of the IMU accelerometer measurements
    L_hat           = X_exp(i-1,17);
    p_i_c_hat       = X_exp(i-1,18:20);
    q_i_c_hat       = X_exp(i-1,21:24);
    ...
    F_d  = ConstructFd(C_q_w_i_hat,Cross_a_x,Cross_omega_x,Cross_omega_x2,delta_t_IMU);
...
%     Q_d= ConstructQdAnalytic(delta_t_IMU,C_q_w_i_hat, Cross_a_x,Cross_omega_x,Cross_omega_x2, var_na, var_nba, var_nomega, var_nbomega, var_nL);
    
    %%-----------------------Stephan Weiss approach -----------------------
    Qd_weiss = calc_Q(delta_t_IMU,q_w_i_hat,omega_m(i,:) - b_omega_hat,a_m(i,:) - b_a_hat,...
        var_na,var_nba,...
    ...
    %% UPDATE / MEASUREMENT
    % This is made in case there is a measurement made in that state
    
    if jj <  length(OpticalFlowTime)
    if IMUTime(i) > OpticalFlowTime(jj) % FIXME: can crash if theres no IMU sample after the last OF sample
        
        if firstOFmeasurement == TRUE
            PREV_measurement_index =i;
            firstOFmeasurement = FALSE;
            continue
        end
        ...
        %% =====================OPTICAL FLOW===================================
        ama=1; % FIXME : get rid of this constant
        
        % Set the pixel position referenced to the center
        x = (OpticalFlow_x(jj,1:matched_feat(jj)))'-Image_width/2+0.5;
        y = (OpticalFlow_y(jj,1:matched_feat(jj)))'-Image_height/2+0.5;
        x = x *(ama/f); y = -y *(ama/f);
        z = ones(size(x));
        
        u = (OpticalFlow_u(jj,1:matched_feat(jj)))'*(ama/f);
        v = -(OpticalFlow_v(jj,1:matched_feat(jj)))'*(ama/f);
        ...
        if length(inliers_idx) < 5
            warning('Inliers information too poor (%d). Avoiding RANSAC selection.\n',length(inliers_idx));
            w = zeros(size(u));
        else
            full_x=x;
            full_y=y;
        ...
        aux = quatmult(X_exp(i,7:10),invquat(X(PREV_measurement_index,7:10)));
        
        rotation = QuatToRotMat(aux);
        ...
        %Scale state
        [vel_IMUOF(jj,:)] = Continuous8point2DoF(x,y,z,u,v,w);
...
plot(IMUTime,X(:,4));grid on
hold on;
plot(IMUTime, vel_truth(:,1),'k');grid on
plot(OpticalFlowTime,1/L_hat*vel_IMUOF(:,1),'-g');
xlabel('seconds'); ylabel('velocity in x [mm/s]');
legend('EKF','Ground truth','Meas');

subplot(3,1,2);
plot(IMUTime,X(:,5));grid on
hold on;
