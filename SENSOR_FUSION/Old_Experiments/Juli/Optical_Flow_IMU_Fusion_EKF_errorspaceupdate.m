...
%     X(i-1,7:10) = ViconPose(index_start,4:7);
    ...
    %Scale state
    [vel_IMUOF(i,:)] = Continuous8point2DoF(x,y,z,u,v,w);
...
fprintf('Result of the Kalman Filter: \n');
fprintf('%d:X(i-1)_vel \t X_exp_vel \t X_err-_vel \t Measurement \t Meas_exp \t X_err_vel \t X_post_vel \t X_gt_vel \n',i);

for ii=1:3
    fprintf('[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\n',...
             X(i-1,3+ii) ,X_exp(i,3+ii), X_err_prior(3+ii),vel_IMUOF(i,ii), z_exp(ii) ,  X_err(i,3+ii)  ,    X(i,3+ii) ,  X_gt(i,3+ii) );    
end

fprintf('%d:rot(i-1) \t X_exp_rot \t X_err-_rot \t  -----  \t  ----- \t X_err_rot \t X_post_rot \t X_gt_rot \n',i);

for ii=1:3
    fprintf('[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\t[%6.5g]\n',...
             X(i-1,6+ii) ,X_exp(i,6+ii), X_err_prior(6+ii),   0  , 0      ,  X_err(i,6+ii)  ,    X(i,6+ii) ,  X_gt(i,6+ii) );    
end
