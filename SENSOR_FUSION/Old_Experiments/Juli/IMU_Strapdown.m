...
function [X_exp,gyromean,accmean] = IMU_Strapdown(Init_State,gyro,acc,IMUTime,RealPosition)
...
X_exp = Init_State';
v_w_i = Init_State(4:6)';
p_w_i = Init_State(1:3)';
q_w_i = quat_normalize(Init_State(7:10))'; 
C_q_w_i = QuatToRotMat(q_w_i);
 

delta_t = IMUTime(end) - IMUTime(1);
% tic 
...
%     a = delta_t_IMU/2*(gyro(j,1) - b_w(1));
%     b = delta_t_IMU/2*(gyro(j,2) - b_w(2));
%     c = delta_t_IMU/2*(gyro(j,3) - b_w(3));
%     
%     q_w_i =      [[ 1  c -b a];
%                   [-c  1  a b];
%                   [ b -a  1 c];
...
% 
%     %Velocity          
%     v_w_i = v_w_i + (C_q_w_i * (acc(j,:)' - b_a) + [ 0 0 G]') *delta_t_IMU;
%     
%     %Position
%     p_w_i = p_w_i + v_w_i * delta_t_IMU;
% end
% 
% gyromean = mean(gyro(2:end,:),1);
...
for j = 2:steps
    ...
    Omega_mean = delta_t_IMU/2 * (Omega + Omega_prev)/2;
    ...
    q_w_i_prev = q_w_i;
    q_w_i = (Exponential + 1/48 *(Omega*Omega_prev - Omega_prev*Omega)*delta_t_IMU^2) * q_w_i_prev;
    
    q_w_i = quat_normalize(q_w_i); % We use the quaternions as row vectors instead of columns

    %%-debug
%     q_w_i = RealPosition(4:7);
    %%-debug
    
    C_q_w_i_prev = C_q_w_i;
    C_q_w_i = QuatToRotMat(q_w_i);
    

    %Velocity         
    v_w_i_prev = v_w_i;
    v_w_i = v_w_i_prev + ((C_q_w_i * (acc(j,:)' -b_a ) + C_q_w_i_prev * (acc(j-1,:)' -b_a ))/2 + [ 0 0 G]') *delta_t_IMU;
    
    %Position
    p_w_i = p_w_i + (v_w_i + v_w_i_prev)/2 * delta_t_IMU;
end

gyromean = mean(gyro(2:end,:),1);
% accmean = (v_w_i - Init_State(4:6)' )/delta_t;
accmean = mean(acc(2:end,:),1);
